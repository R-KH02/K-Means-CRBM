function [images] = display_crbm_v2_bases(W, V1, expandfactor, opt_nonneg, cols)
...
for i=1:size(W, 3)
    ...
    if isempty(images), images = zeros(size(negdata_expand,1), size(negdata_expand,2),size(negdata_expand,3), size(W,3)); end
    images(:,:,:,i) = negdata_expand; %#ok<AGROW>
end

if exist('cols', 'var')
    display_network(reshape(images+eps, size(images,1)*size(images,2), size(images,3)), true, true, cols, false);
else
    display_network(reshape(images+eps, size(images,1)*size(images,2), size(images,3),size(images,4)), true, true);
end
